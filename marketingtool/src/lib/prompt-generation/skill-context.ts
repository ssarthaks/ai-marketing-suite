...
export function getRelevantSkills(messages?: any[]): string[] {
...
export async function loadRelevantSkills(messages?: any[]): Promise<string> {
  const relevantSkillNames = getRelevantSkills(messages).slice(0, 3);
  ...
  try {
    ...
    console.warn("Could not read skills from DB:", e);
  }

  return skillEntries.join("\n\n---\n");
}

export async function buildSkillsBlock(messages?: any[]): Promise<string> {
