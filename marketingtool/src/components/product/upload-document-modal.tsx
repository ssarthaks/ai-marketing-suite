...
export function UploadDocumentModal({
  ...
  const handleFileSelect = (selectedFile: File) => {
    ...
    if (!validExts.includes(ext)) {
      toast.error(`Unsupported file type (.${ext}). Please upload PDF, DOCX, TXT, MD, CSV, or JSON files.`);
      return;
    }

    if (selectedFile.size > 7 * 1024 * 1024) {
      toast.error("File size exceeds the 7MB limit.");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Failed to update context via AI:", err);
      toast.error(err.message || "Failed to update context with document.");
    } finally {
      setIsProcessing(false);
      setStatusMessage("");
    }
      ...
      <DialogContent className="max-w-xl bg-background border border-border shadow-2xl rounded-2xl p-6 sm:p-7">
        <DialogHeader className="gap-1.5 pb-3 border-b border-border/60">
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            Upload an updated product document (PDF, DOCX, TXT, MD) and let DeepSeek AI parse and synthesize it into your product context markdown file.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-5 py-3">
          <div className="flex items-center justify-between bg-muted/40 px-3.5 py-2 rounded-lg border border-border/60 text-xs">
            <span className="text-muted-foreground font-medium">Target Context File:</span>
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0 flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {(file.size / 1024).toFixed(1)} KB • {file.name.split(".").pop()?.toUpperCase()} Document
                    </span>
                  </div>
                </div>
                {!isProcessing && (
              ...
              <div className="flex flex-col items-center text-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Supports PDF, DOCX, TXT, MD, CSV, JSON (max 7MB)
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                <div className={`size-4 rounded-full border flex items-center justify-center mt-0.5 shrink-0 ${mode === "smart_merge" ? "border-[#6FB941] bg-[#6FB941]" : "border-muted-foreground"}`}>
                  {mode === "smart_merge" && <div className="size-1.5 rounded-full bg-white" />}
                </div>
                <div className="flex flex-col gap-0.5">
                  ...
                  <span className="text-[11px] text-muted-foreground leading-tight">
                    Combines new document details with existing context while preserving valid sections.
                  </span>
                </div>
              </button>
              ...
              >
                <div className={`size-4 rounded-full border flex items-center justify-center mt-0.5 shrink-0 ${mode === "replace" ? "border-[#6FB941] bg-[#6FB941]" : "border-muted-foreground"}`}>
                  {mode === "replace" && <div className="size-1.5 rounded-full bg-white" />}
                </div>
                <div className="flex flex-col gap-0.5">
                  <span className="text-xs font-semibold flex items-center gap-1.5 text-foreground">
                  ...
                  <span className="text-[11px] text-muted-foreground leading-tight">
                    Re-creates the context document strictly using the uploaded file as source of truth.
                  </span>
                </div>
              </button>
            </div>
          ...
          {isProcessing && (
            <div className="flex items-center gap-3 p-3 bg-muted/50 border border-border/80 rounded-xl text-xs text-foreground animate-pulse">
              ...
              <span>{statusMessage || "Processing document with DeepSeek AI..."}</span>
            </div>
          )}
        </div>
