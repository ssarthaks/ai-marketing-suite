...
export function setPendingInitialMessage(msg: typeof pendingInitialMessage) {
  pendingInitialMessage = msg;
}
...
export function ChatView({ threadId, initialModelId }: Props) {
  ...
  const [modelId, setModelId] = useState<ChatModelId>(
  ...
  const runModel = useCallback(
    async (afterMessages: ChatMessage[], targetThreadId?: string) => {
      ...
      try {
        ...
        const fallback = `I'm having trouble reaching ${activeModel.label} right now.\n\nIf this keeps happening, confirm that your **${activeModel.apiKeyName}** is set on the server and restart the dev server.`;
    ...
    [modelId, setMessages, updateThread],
  );
  ...
  // Automatically sync pro model access from DB without requiring a re-login
  useEffect(() => {
    if (session?.user && session.user.role !== "admin") {
      checkProAccess()
        .then((hasAccess) => {
          if (hasAccess !== !!session.user.pro_model_access) {
            updateSession({ pro_model_access: hasAccess });
          }
        })
        .catch(() => {});
    }
  }, [session?.user?.pro_model_access, session?.user?.role, updateSession]);
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            session?.user?.role === "admin" ? (
              ...
              <AlertDialogAction
                className="bg-[#6FB941] hover:bg-[#5da035] text-white"
                disabled={session?.user?.pro_model_requested || isRequesting}
                onClick={async (e) => {
                  e.preventDefault();
                  if (session?.user?.pro_model_requested) return;

                  setIsRequesting(true);
                  try {
                    await requestProModelAccess();
                    await updateSession({ pro_model_requested: true });
              ...
              >
                {session?.user?.pro_model_requested
                  ? "Requested"
                  : isRequesting
                    ? "Requesting..."
                    : "Request Access"}
