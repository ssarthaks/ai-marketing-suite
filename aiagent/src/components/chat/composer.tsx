...
export function Composer({
  ...
  const displayPlaceholder = hoveredPrompt
    ? hoveredPrompt.length > 120
      ? hoveredPrompt.slice(0, 170).trim() + "..."
      : hoveredPrompt
    : placeholder;
  ...
  const handlePickFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      if (!res.ok) {
        const parsedError = uploadErrorSchema.safeParse(await res.json());
        throw new Error(
          parsedError.success ? parsedError.data.error : "Upload failed",
        );
      }
      ...
      const uploadedAttachments: ChatAttachment[] = data.results.map(
        (result) => ({
          ...
          size: result.size,
          url: result.url,
          previewUrl: result.type.startsWith("image/") ? result.url : undefined,
        ...
        <div className="flex w-full items-center justify-between gap-2 px-4 pb-3 pt-1">
          <div className="flex items-center gap-1 min-w-0">
            ...
            <input
              ...
              type="file"
              multiple
              accept=".pdf,.doc,.docx,.pptx,.xlsx,.txt,.md,.csv,.json,.tsv,.png,.jpg,.jpeg,.gif,.webp"
              className="hidden"
