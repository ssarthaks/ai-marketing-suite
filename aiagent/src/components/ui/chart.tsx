...
function safeCssIdentifier(value: string): string | null {
...
function safeCssColor(value: string | undefined): string | null {
  if (
    ...
    value.length > 128 ||
    /[;{}<>\\/"']/u.test(value) ||
    /(?:expression|url|@import)/iu.test(value)
  ) {
    return null;
  }
  return value;
...
>(({ id, className, children, config, ...props }, ref) => {
  ...
  const requestedId =
    typeof id === "string" ? safeCssIdentifier(id.slice(0, 64)) : null;
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  const colorConfig = Object.entries(config).filter(
    ([key, config]) =>
      safeCssIdentifier(key) && (config.theme || safeCssColor(config.color)),
  );
